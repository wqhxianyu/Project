...
public class ObjectBeanService {
	...
	public static int intsertObjectBean(ObjectBean bean) throws Exception {
		...
		try {
			...
			sql = "insert into wj_object(title,discribe,createtime,state,remark,anonymousFlag,name) values(?,?,?,?,?,?,?)";
			System.out.println(sql);
			stmt = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
			stmt.setString(1, bean.getTitle());
			stmt.setString(2, bean.getDiscribe());
			stmt.setTimestamp(3, bean.getCreateTime());
	...
	public static List ListObjectBean(String username) {
		...
		try {
			...
			if(username.equals("admin")){
				sql = "select oid,title,createtime,state,remark,name,discribe from wj_object order by oid desc ";
			}else{
				sql = "select oid,title,createtime,state,remark,name,discribe from wj_object where remark = '"+username+"' order by oid desc ";
			}
			...
			while(rs.next()){
				...
				ob.setDiscribe(rs.getString("discribe"));
				objList.add(ob);
			}
		} catch (Exception e) {
			e.printStackTrace();
