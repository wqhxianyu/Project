...
//	 根据题目顺序查找选项的值 返回一个结果集
	...
	public List listSelecterBySeq(int seq, int oid) {
		...
		String sql = "select qseq,selseq,content,remark from wj_selecter where qseq = '"
		...
		try {
			...
			while(rs.next()){
				...
				sel.setRemark(rs.getString("remark"));
				sel.setQseq(qseq);
...
//	 插入选项
	public int addSelecter(int oid, int seq, String content, int seq_selecter,String score) {
		DBConnection dbcon = null;
		Connection con = null;
		Statement stm = null;
		ResultSet rs = null;
		String sql = "insert into wj_selecter(oid,qseq,content,selseq,remark) values('"
				+ oid
				+ "','"
				+ seq
				+ "','"
...
//	 传进来问卷编号和试题的顺序号 删除该题所对应的选项表中的数据
	...
	public void syscore(String score){
