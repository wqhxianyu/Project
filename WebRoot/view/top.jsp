...
<table width="100%" border="0" cellpadding="0" cellspacing="0" background="skin/images/frame/topbg.gif">
  <tr>
    ...
    <td width='80%' align="right" valign="bottom">
      ...
      <%
      if(session.getAttribute("screen_name").equals("admin")){%>
      	您好，<span class="username"><%=session.getAttribute("screen_name") %></span>，欢迎使用内容管理系统！
        	[<a href="./modifyPsw.jsp" target="main" >修改密码</a>]
        	[<a href="user/exit.jsp" target="_top">注销退出</a>]&nbsp;
     <%}else{
      %>
        	您好,<a href="//weibo.com/u/<%=session.getAttribute("uid") %>" target="_blank">
		    <img alt="<%=session.getAttribute("screen_name") %>" src="http://tp1.sinaimg.cn/<%=session.getAttribute("uid") %>/30/5650501886/1"></img></a>
		    <a href="http://weibo.com/u/<%=session.getAttribute("uid") %>" target="_blank"><span class="username"><%=session.getAttribute("screen_name") %></span></a>，欢迎使用内容管理系统！
        	<!-- [<a href="./modifyPsw.jsp" target="main" >修改密码</a>]
        	[<a href="user/exit.jsp" target="_top">注销退出</a>]&nbsp; -->
        	<%} %>
      </td>
